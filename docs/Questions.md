...
「键盘设置」->「键盘布局」->「中文26键」，点击「>」进入键盘详细设置页面，点击「划动设置」标签栏，你可以为已有按键设置上下左右不同划动设置。

如果你是要为新增空格两侧的按键设置划动，也是可以的。

1. 点击右上角「+」新增一个 “Action”，如果空格两侧新增按键勾选了「按键由 RIME 处理」，那么就选择「字符」，否则选择「符号」。
2. 在弹出框中填写你新增空格两侧按钮填写的符号，点击「保存」。
3. 在列表中找到你刚新增的 Action，点击进入划动设置详细页面。
4. 在此页面中添加你想设置的上下左右划动配置。
...
1. 新增 `hamster.custom.yaml` 文件，文件内容如下：
    ...
    > 注意：如果你自身已经有 `hamster.custom.yaml`文件了，这可以上面的 `patch`以下内容（不包含 `patch`，注意缩进）添加到自己的配置文件中。
...
`40`的距离，经过测试不容易触发划动，除非刻意触发。同时适当调节正切值。上面两个参数的值是已是默认值，是测试不容易误触的阈值了，如过对于您还不理想，您可以在适当调节。
...
如[文件管理-「应用文件」与「键盘文件」](FilesManager.html#应用文件与键盘文件)中所描述的，默认情况，在 `AppGroup` 目录下键盘只有读取权限，无写入权限，所以键盘生成的自造词文件是无法保存的。
